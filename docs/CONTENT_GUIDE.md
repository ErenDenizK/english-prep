...
Optional in the schema, and present on every shipped topic — because it
is no longer a side door. `#egitim/konu/<topicId>` **is** the topic
screen: tapping a topic on the Eğitim index lands here, and the topic's
lesson rows live on it, one level below the index. It used to be reached
from a "Bu konu nedir?" button under a topic heading on a flat index of
every lesson in the app; that index stopped existing when it was
measured at 5,332px — 8.3 screens at 320 — and became a list of ten
topics instead (`js/education.js:740`).

So a topic without an `intro` now ships a screen whose first half is
missing. `npm run format` still sets `hasIntro: true` on the manifest
entry, so the index knows without fetching this file.
