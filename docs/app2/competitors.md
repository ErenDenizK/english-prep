...
§3.3 has it. Against Kwiziq — the only incumbent whose mechanism is the
...
named the intermediate plateau for Cambridge in 2008, the
heritage-speaker literature describes this exact profile, and Duolingo
publishes blog posts about the plateau it cannot fix [S]. What I could
not find is anyone paying for a product aimed at it (§3).
...
before buying (§5.1) — and "the diagnostic" is the most-copied artefact
in the category, given away as a lead magnet by EF, Cambridge, Oxford,
test-english.com and Kwiziq alike (§5.2). What survives is narrower and
better: **a taxonomy of contrasts plus a stated refusal to assert more
than the evidence supports** — a content property, not a feature.

**6. The positioning sentence names the learner, not the mechanism**
(§6) — and must never contain *personalised*, *AI coach*, *study plan*,
*master*, *fluent*, or a CEFR level.
...
and Kwiziq localises by region [S], so where sources disagree I print
both. **Nothing here is a pricing input until someone opens the live
page on the storefront that matters.**

**Complaint evidence is aggregator-filtered, not primary.** Search
returned almost no Reddit or forum voices directly; what came back was
synthesised from review blogs, many affiliate-monetised and biased
toward "worth it". The only near-primary signal was Trustpilot
aggregates, which skew negative by construction.

**Market-size figures are worthless.** Estimates for the 2026
English-learning market: $10.7B, $13.0B, $20B, $24.0B, $68.4B [S] — a
factor of six between research houses, which means the category has no
agreed boundary and anyone quoting one number has picked it.

---

## 1 · The teardown

Ordered by how directly each competes for the learner in question: an
adult who already functions in English and wants the scaffolding named.
...
revenue, 12.7M paid subscribers [S] — about 22% of DAU paying. It cannot
model this learner at all: its representation of a user is a position on
a path, encoding *how far through our content are you* rather than *what
do you get wrong*.
...
relevant Tips and Notes"* [S]. **The competitor with 58.7 million daily
users deleted the artefact App 2 is built out of, and its users
objected** — the best demand signal in this document, and a warning,
because the removal was deliberate and presumably measured. [≈]

### 1.2 Busuu

Lessons inside CEFR levels A1–B2, built with McGraw-Hill; progress is
level completion plus a certificate [S]; Premium $9.99/month or
$69.99/year [S, Sep 2026]. Its signature is the **community correction
loop** — you write, natives correct you, you correct theirs — reported
as valuable and unreliable: *"sometimes a correction lands in minutes;
sometimes it sits for a day, or you get a one-word 'good'"* [S].

It sells a level label to someone who already has one; most of this
audience would test B2 and learn nothing, and B2 is Busuu's ceiling. The
community loop is worth wanting and impossible to copy: a two-sided
marketplace is the one asset a solo project cannot bootstrap.
...
The $299 lifetime is the number App 2's pricing must survive — comparing
a finite English product against *all Babbel, all languages, forever,
$299* is arithmetic the plan has not done. [≈]

### 1.4 LingQ

The input-driven Krashen reader, about $10/month annual [S, Sep 2026].
The unit is a word coloured by status — blue new, yellow learning, white
known — and reviewers call the colour-coding *"genuinely motivating… in
a way that abstract percentage scores don't"* [S]. It refuses on
principle to do the one thing this learner needs: *"if you want scripted
lessons and heavy grammar explanations, you'll hit limits fast"* [S].
For someone whose gaps were *caused* by input-only acquisition,
prescribing more input prescribes the disease.

Note the mechanic though: **a per-item state, shown as colour, across a
corpus the learner recognises** — the Brainmap again, in a third
costume, and the best-designed progress display here.
...
requires you to know your gaps already — the purest demonstration of App
2's own claim that *a scheduling engine without a taxonomy cannot find
the boundary you never noticed*, and free, which is why that claim is
worth testing rather than assuming.

### 1.6 Elsa Speak, and Speak

Both target speech and both mark the edge of App 2's scope. **Elsa**
scores pronunciation at phoneme level for $159.99/year [S, Sep 2026];
reviewers note its American-English orientation and that some find *"the
...
then runs them through an AI tutor, at $83.99/year [S, Sep 2026]; backed
by the OpenAI Startup Fund at roughly $1B after a $78M Series C in late
2024 [S]. Both drill the one skill this audience has — someone who
learned English from Discord can talk; what they cannot do is write a
sentence that survives a professor. Speak also answers App 2's own
...
is careful: it helps *"when used alongside actual grammar
instruction"* but is *"not specifically designed as a teaching tool"*
[S]. It removes the error without naming the boundary: a learner who
writes *"I am working here since 2019"*, accepts the fix to *"have been
working"* and closes the tab has had a symptom treated and has not been
told that *since* selects a perfect. They will do it again tomorrow.
...
reference [S]; BBC Learning English is still active in 2026 [S];
test-english.com gives free level and topic tests at A1–B2 [S];
UsingEnglish lists 297 free quizzes [S]. They have no model of *you*:
they are a library, and this learner's problem is not that the library
is shut but that they do not know which shelf to walk to.
...
units and sells the other 139 of 145 by in-app purchase [S]. The same
145 units, in the same order, for everyone. **Murphy is the thing the Brainmap was invented to improve on,
and the thing App 2 is also trying to improve on. Both are answering
Murphy; one has been doing it for ten years.**

### 1.10 Gymglish

A 10–15 minute lesson delivered daily by email or app, from €14/month
[S, Sep 2026], with an adaptive revision queue and an assessment showing
*"current strengths and weaknesses, past improvement and future revision
needs"* [S]. Its unit is the *day*, not the gap: content scheduled by
calendar rather than by error. It is also the product that solved App
2's retention problem without a streak — email delivery, no punishment,
no counter. **And "strengths and weaknesses" and "future revision needs"
are already a competitor's feature bullet**, so that phrasing is not
available to App 2 as a differentiator.

### 1.11 Tutoring: Preply, italki, engVid

Preply and italki sell an hour of a person: Preply $3 to $40+/hour
averaging $10–15; italki $4 to $60+ [S, Sep 2026]. Progress is whatever
the teacher tracks, which is durably nothing. An hour with a good
teacher is the most effective intervention in this document and almost
nobody buys twenty of them — the failure is cost, scheduling, and the
mild embarrassment of paying someone to watch you get *few / a few*
wrong. But notice what a tutor does in the first fifteen minutes:
**they diagnose.** App 2's pitch is the first fifteen minutes of a good
tutor, at software prices, repeatable — a better analogy for the product
than anything in the app tier, and §6 uses it.

**engVid** is the free video tier: 2,300+ lessons from nine teachers
with their own YouTube channels, plus a quiz per video [S]. No
persistence, no model of the learner, and discovery by browsing a wall
of titles — which again requires you to already know your gap.
...
**What it does badly** is what `two-apps.md` §3.3 said: you must bring
it the question, and it will never raise a boundary you have not
noticed. What that section undersells is that **it is agreeable.** A learner who asks
*"isn't 'such smooth' fine too?"* risks being told yes. For an audience
defined by a good ear and no labels, a confidently agreeable tutor is
not neutral — it is a machine for fossilising the exact errors App 1's
blind pass exists to catch. [≈]
...
A London company founded by Gareth Davies and Simon Potter, self-funded
until 2015 and having raised about $313K in one round since [S] — two
names, one small raise, ten years of operation, which is roughly the
shape this project could reach. You take a **placement test**,
grammar-only, mapped to CEFR A1–C1 with priority given to intermediate
content [S]. "Kwizbot" then *"starts to build up a picture of what you
know, don't know or may have mislearned"* [S] and generates a
**StudyPlan**; after each short quiz it decides whether to keep the
current topics or pick new ones, and *"can occasionally bring back
lessons so that you don't forget things you learned earlier"* [S].
...
The interactive Brainmap is premium [S], and the French course covers
*"all 500 French grammar topics organised by level and area"* [S].
...
**That is mostly good news.** It is prior art that the design teaches
and that people pay for: `vision.md` §3 worried that a "get better at
English" app has no stopping rule and no honest retention story, and
Kwiziq answers both empirically — the per-point state *is* a visible
stopping rule, and it has sustained a subscription for ten years without
a streak. Its complaints are also a free defect list (§4.2).

**The bad news is exactly one thing: it removes "nobody has built this"
from the pitch**, and it should leave the plan's own self-description.
`vision.md` §3 presents the boundary model as the solution to a problem
the category has not solved; the correct sentence is narrower — *this
design is proven, and it has never been pointed at English.*
...
resolved in the direction App 2 has refused. This is the one difference
that is also a weapon.

**3. No levels.** Kwiziq's whole frame is CEFR; App 2's audience does
not describe itself with a level, so refusing CEFR is a real divergence
aimed precisely at *competence without labels*. Everything else —
diagnostic, map, colour, re-probe, per-point lesson, honest "not tested
yet" — is Kwiziq's.
...
~100 boundaries at 6–8 minutes of undelegable attention each. **There is
no version of this where App 2 out-produces Kwiziq**, so the only
defensible positions are the ones content volume cannot buy: the
language it is not in, the price it cannot charge, and the claim it
cannot make.
...
pointed it at English?** Italian, Portuguese and German were on their
own roadmap in 2017 and none shipped [S], so the constraint is not
ambition. Two hypotheses, and the second should worry this project. [≈]
**A: content cost** — each language is a decade of expert authoring,
which also explains the unshipped Italian. **B, and I think this is the
real one: the customers are different people.** Kwiziq's learners are largely Anglophone adults learning French
...
market has the opposite economics** — largest by headcount, among the
lowest by revenue per user, which is why Duolingo Super sells at
...
`vision.md`'s anchor is already right (per storefront, against local
Duolingo), but the honest expected revenue at that anchor is far below
what the same product earns in Kwiziq's market. **The strongest argument
against App 2 here is not competitive; it is that the audience it chose
is the one the proven business model avoids.** Not fatal for a project
whose ceiling is hundreds of users and whose author says profit is not
the point — fatal for any plan that quietly assumes otherwise.
...
effort devoted to correcting them"* [S]. He calls it *temporary
fossilization*, escapable with the right pedagogy [S]. The
**heritage-speaker** literature `two-apps.md` §2.1 cited describes the
same shape from the other side — *"advanced listening skills, strong
conversational instincts, deep cultural knowledge… what they may lack
are formal registers, academic vocabulary, or literacy training"* [S] —
and Duolingo itself publishes on the intermediate plateau [S], the
category leader documenting the ceiling of its own method.

**What they currently do is leave.** The recurring advice is *outgrow
Duolingo, then switch to input*: LingQ, native content, Anki, a tutor if
affordable, reference sites when a question arises. There is a thin tier
marketing to them directly — a "Moreover — Advanced English" app, B2–C1
Substacks, twelve-week "break the plateau" guides on Gumroad [S] — and
**the thinness is the finding**: a cottage industry of one-person
products, which is precisely what App 2 would be. What they say is
missing — explanations of *why* an answer was wrong, content that is not
beginner-shaped, something that says where to aim — maps onto App 2's
three claims, which is either validation or my own search terms coming
back to me, honestly partly the latter. [≈]

**The gap between "real segment" and "real market."** About 1.5 billion
people are learning English [S], but I found no figure sizing *this*
segment and no product reporting revenue from it. `two-apps.md` §2.2
said the segment has no deadline, and the plateau literature sharpens
that, because a plateau is by definition a state of *functioning
adequately*. **The pain is real and low-grade; low-grade pain converts
badly; and the products serving it are one-person products.** That is
consistent with a real segment that will not support a company — fine
for a project whose target is a few hundred people who would be upset if
it disappeared, and not something to write up as an underserved market
opportunity.
...
Users report *"maintaining their streak just to pass challenges rather
than focusing on actual learning"*, and *"if they realize they're about
to lose it at the end of the day, they'll spend 5 minutes on an old
mastered lesson just to save it"* [S]. On motivation: *"because of
Duolingo's emphasis on external motivation, many users don't develop
their intrinsic motivation… the streak becomes more important than
learning Spanish itself"*, with the claim that *"streaks will undermine
intrinsic motivation after a while"* [S]. And the compulsion register:
1,139-day streaks kept *while wanting to quit*, with *"not losing the
streak the only thing pushing them to complete daily lessons"* [S].
...
it is a values position, not a demand signal — and §5.4 prices it.

### 4.2 Kwiziq's complaints, which are App 2's future complaints

A defect forecast, not a competitor's problems: App 2's design produces
...
- **Weak on vocabulary**, with reviewers recommending Memrise alongside
  [S]. App 2's boundary model shares that blind spot by construction.
- **No speaking, limited advanced content, price, and interface
  confusion** — *"so complicated and confusing… didn't find the lessons
  helpful at all"* [S].
...
**The last one is the cheapest lesson here.** A small subscription
product with an unclaimed review profile and an unanswered refund
complaint is one Sunday-morning email from a 1-star trend — the exact
message `vision.md` §4 prices as the cost of charging money.

### 4.3 Advanced learners on beginner-shaped products

Softer evidence, not to be overstated: apps *"not for advanced
learners"*, most topping out around intermediate [S], and the direct
version — apps for adults are *"built to maximise activity rather than
real-world progress"*, leaving learners unable to *"review weak spots
efficiently"* [S]. **The sharpest single data point remains Duolingo's
removal of Tips and Notes** (§1.1), because it is a behaviour rather
than an opinion.
...
answer fell in the failing band** — the definition of a credence good,
quality unverifiable before and often after purchase. A learner who gets
a wrong explanation from a chat box and a right one from App 2
experiences two confident explanations. Twenty-eight hours of
cold-solving buys something real and buys nothing a landing page can
prove; **the only way correctness becomes perceivable is to make the
*process* visible rather than the outcome.** [≈]
...
lead magnets: EF SET's 15-minute check, Cambridge's free 25-question
placement test, the Oxford Online Placement Test, test-english.com's
tests at five levels, and Kwiziq's own [S]. A learner has met four of
these; "take our diagnostic" is not a new offer, it is the thing they
skip.
...
the product, not the list. **And the output is unpleasant**: it hands an
adult who believes they are good at English a list of things they get
wrong, where Duolingo sells a green owl partly because the alternative
is telling adults they are wrong, which people pay to avoid. [≈]
...
2023 and 2025, against documented subscription fatigue [S].
...
compounds it. Fine — provided the plan does not later read poor download
numbers as a product problem when they are a strategy consequence.
...
Turkish, Brazilian, Indonesian, Polish. Stores rank per storefront and
per language, and the query that finds this product is typed in the
learner's own language. **Localisation survives — in the listing, the
keywords and the first screenshot** — a much smaller job than
re-authoring 37,000 words and a much more consequential one. The store
listing is not a v2.0 nicety; it is the v1.0 acquisition channel. [≈]
...
— and **the taxonomy plus the honesty rules**: contrast-shaped units,
evidence-bounded claims, and an item standard that refuses distractors a
competent teacher would accept. Content properties, expensive, slow, and
not copyable without spending the same attention. Neither is "we explain
better"; neither is "we have a diagnostic".
...
here a subscription business cannot copy (§2.4).

Two supporting lines: **the tutor analogy** (§1.11) — *the first fifteen
minutes with a good teacher, done properly and repeatable* — which
explains the diagnostic without using the word; and **the evidence
line**, the real §5.2 wedge — *not "you don't know what you don't know",
but "here are the eight you hadn't noticed, and the sentence that proves
each one."*
...
- **"Master English grammar."** Every grammar app, and wrong on the
  ...
  `CLAUDE.md`'s rule is *refine, not teach from zero*.
- **"Go from B2 to C1", or any CEFR promise.** A product that refuses
  levels cannot sell one (§2.4). Likewise **"fluent in N months"** and
  **"speak with confidence"** — owned by Speak, Elsa and every tutoring
  marketplace, and promises this product cannot keep.
...
1. **Take the Kwiziq French level test on the free tier and look at the
   real Brainmap.** An hour, no money, and every claim in §2 becomes
   first-hand — including whether the grey "not tested yet" state is as
   honest in practice as the copy says.
2. **Check Kwiziq's live price from a Turkish IP.** §2.6 turns on
   whether Kwiziq localises downward the way Duolingo does.
3. **Map App 1's 60 categories onto Kwiziq's French topic list.** If
   most have a counterpart, §2.4's "sharper taxonomy" claim should leave
   the pitch.
4. **Run the v0.1 diagnostic on fifteen strangers and count how many
   ...
   §5.2, and what `two-apps.md` §2.5 already asked for.
